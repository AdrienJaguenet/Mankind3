...
{
	return v3_norm(vec3(
		  sinf(camera->rotation.x) * cosf(camera->rotation.y),
...
void setup_camera(program_t* program, SDL_Window* window, Camera * camera)
{
	...
	mat4_t view = m4_look_at(camera->position, v3_add(camera->position, get_Camera_lookAt(camera)), get_Camera_up(camera)),
		   projection = m4_perspective(90, 1.0f*view_width/view_height, 0.01f, 1000.0f);

	glUniformMatrix4fv(glGetUniformLocation(program->id, "view"), 1, GL_FALSE, (float*)&view);
	glUniformMatrix4fv(glGetUniformLocation(program->id, "projection"), 1, GL_FALSE, (float*)&projection);
}

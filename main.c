...
void try_move(AABB * aabb, vec3_t diff, Map * map)
{
					  ...
					  Map * map, unsigned int delta_ticks)
{
	...
	if (keystates[SDL_SCANCODE_W]) {
		try_move(player,
				 v3_muls(get_Camera_forward(camera), 0.02 * delta_ticks), map);
	}
	...
	if (keystates[SDL_SCANCODE_SPACE]) {
		try_move(player, vec3(0.0, 0.05 * delta_ticks, 0.0), map);
	}
}
...
int main()
{
	...
	while (running) {
		...
		try_move(&player, vec3(0.0, -0.01 * delta_ticks, 0.0), map);
